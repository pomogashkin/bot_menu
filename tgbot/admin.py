    ...
    def broadcast(self, request, queryset):
        ...
        if 'apply' in request.POST:
            ...
            if len(queryset) <= 3 or DEBUG:  # for test / debug purposes - run in same thread
                for u in queryset:
                    utils.send_message(
                        user_id=u.id, text=broadcast_message_text, parse_mode=telegram.ParseMode.MARKDOWN)
                self.message_user(
        ...
        return render(
            request, "admin/broadcast_message.html", {
                'items': queryset, 'form': form, 'title': u' '}
        )
...
class ShoppingCartAdmin(admin.ModelAdmin):
    """Управление объектами ShoppingCart в админ-зоне."""
